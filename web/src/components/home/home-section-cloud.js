...
const MarqueeLine = styled.div`
  ...
  .hoverBlue {
    a:hover {
      color: ${colors.FATurquoise};
    }
  }
  .hoverGreen {
    a:hover {
      color: ${colors.FADarkerBlue};
    }
  }
  .hoverOrange {
    a:hover {
...
const TagCloud = React.forwardRef((props, ref) => {
  ...
  useEffect(() => {
    let ctx = gsap.context(() => {
      ...
      gsap
        .to(".marqueeContent1", { xPercent: -100, repeat: -1, duration: 140, ease: "linear" })
        .totalProgress(0.6);
      gsap
        .to(".marqueeContent2", { xPercent: -100, repeat: -1, duration: 160, ease: "linear" })
        .totalProgress(0.4);
    }, ref);
    return () => ctx.revert();
  }, []);
